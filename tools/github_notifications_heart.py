...
def _write_dlq(event_path: Path, reason: str, *, dry_run: bool) -> None:
    ...
    try:
        ...
        dest = DLQ_DIR / f"{event_path.name}.dlq"
        # Collision-resistant naming: append random hex to handle events
        # processed in the same second with identical stems (e.g. two
        # '20260901T120000-ev-001.json' files regenerated by iot). The loop
        # bounds at 10 attempts to avoid pathological infinite loops.
        for _attempt in range(10):
            if not dest.exists():
                break
            dest = DLQ_DIR / f"{event_path.stem}-{_now_iso().replace(':', '')}-{secrets.token_hex(3)}-{reason}{event_path.suffix}.dlq"
...
def get_status() -> dict[str, Any]:
    ...
    if DLQ_DIR.is_dir():
        with contextlib.suppress(OSError):
            # Exclude hidden files (OS thumbnail/database files) from the count.
            dlq_total = sum(1 for p in DLQ_DIR.iterdir() if p.is_file() and not p.name.startswith("."))
    last_event_at = None
    if state.is_file():
        try:
