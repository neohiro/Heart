...
def _enqueue_poke(state: CycleState, kind: str, payload: dict[str, Any]) -> None:
    ...
    try:
        ...
        target = queue_dir / f"{ts_slug}_{pid_slug}_{mono_slug}_{kind}.json"
        import tempfile
        tmp = None
        try:
                ...
                if not org_path.is_file():
                    ...
                    if auto_create and not DRY_RUN:
                        try:
                            ...
                            org_path.write_text(
                                f"# org-{org} — auto-generated by Heart.self_reflexive_check\n\n"
                                f"> **Status: machine-generated skeleton.** Edit to add real content.\n\n"
                                f"## identity\n\n  - org: {org}\n  - authority: unknown\n  - summary: TODO\n  - scope: TODO\n",
                                encoding="utf-8",
                            )
...
def _phase_prune_shared(state: CycleState) -> PhaseResult:
    ...
    Only prunes files in the designated transient subtrees:
      - heartbeat/observations/          (OSINT raw observations — replayable)
    ...
    if not DRY_RUN and triggered and files_pruned > 0:
        try:
